...
def find_tracked_file_by_name(basename_to_find: str, server_dir: Path, is_repo: bool) -> str | None:
    ...
    try:
        ...
        if result.returncode != 0:
             # Return code 1 just means no files matched the pattern, not necessarily an error
        ...
        if len(matches) == 1:
            # print(f"Info: Found unique tracked file match: '{matches[0]}'", file=sys.stderr) # Less verbose
            return matches[0]
        elif len(matches) > 1:
            print(f"W: Ambiguous filename marker '{basename_to_find}'. Found multiple tracked files: {matches}.", file=sys.stderr)
            return None
        else: return None # No matches found
...
def is_git_tracked(filepath_relative_to_repo: str, server_dir: Path, is_repo: bool) -> bool:
    ...
    try:
        ...
        # Use check=True to raise CalledProcessError if not tracked (or other error)
        subprocess.run(command, capture_output=True, text=True, check=True, encoding='utf-8', cwd=server_dir, timeout=5)
        return True
    except subprocess.CalledProcessError:
        return False # Command returned non-zero, meaning file is not tracked or doesn't match
    except (subprocess.TimeoutExpired, Exception) as e:
...
def update_and_commit_file(filepath_absolute: Path, code_content: str, marker_filename: str, server_dir: Path, is_repo: bool) -> bool:
    ...
    try:
        ...
        if filepath_absolute.exists():
             ...
             except Exception as read_e: print(f"W: Could not read existing file {git_path_posix} to check changes: {read_e}", file=sys.stderr)
        # Compare content exactly as it will be written
        if code_content == current_content:
             print(f"Info: Content for '{git_path_posix}' identical. Skipping Git actions.", file=sys.stderr)
             return True # Report success as no action needed
        ...
        if not save_code_to_file(code_content, filepath_absolute):
            print(f"E: Failed to save '{git_path_posix}' before Git operations.", file=sys.stderr)
            return False # Stop if saving failed
        ...
        if add_result.returncode != 0:
            ...
            # Consider attempting to revert the save? For now, just report failure.
            return False

        # Commit the specific file
        commit_message = f"Update {marker_filename} from AI Code Capture"
        # Using '--' ensures the path isn't misinterpreted as a revision or option
        commit_command = ['git', 'commit', '-m', commit_message, '--', git_path_posix]
        ...
        if commit_result.returncode == 0:
            ...
            return True
             ...
             if any(p in combined_output for p in no_changes_patterns):
                 ...
                 # Consider reverting the add/save?
                 return False
    except Exception as e:
