...
def find_unique_filepath(suggested_filename: str) -> str:
    """Finds a unique path in SAVE_FOLDER, appending _1, _2, etc."""
    base, ext = os.path.splitext(suggested_filename)
    counter = 1
    filepath = Path(SAVE_FOLDER) / suggested_filename
...
def submit_code():
    if request.method == 'OPTIONS': return '', 204 # Handle preflight
    ...
    if request.method == 'POST':
        ...
        if match:
            ...
            if sanitized:
                ...
                if not file_ext or len(file_ext) < 2:
                     ...
                     sanitized = base_name + detected_ext
        ...
        # Save the potentially modified code (marker removed if found)
        try:
            os.makedirs(SAVE_FOLDER, exist_ok=True)
        ...
        # Check Syntax and Optionally Run ONLY if it's Python
        syntax_ok = None
        run_success = None
        log_filename = None
